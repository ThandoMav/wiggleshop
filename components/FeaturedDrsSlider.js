...
const FeaturedDrsSlider = ({drs}) => {
  ...
  return (
    ...
    >
      {drs.map((program, idx) => {
        // destructure program
        const { image1, name, category, city, slug } = program;
        return (
          <SwiperSlide
            className="max-w-[320px] max-h-[320px] relative "
            key={idx}
          >
            <Link href={`/best-doctor/${slug}`}>
              <div
              ...
              >
                ...
                <div className="flex items-center justify-between mt-8 lg:mb-0">
                  ...
                  <Link className="font-medium" href={`/best-doctor/${slug}`}>
                    Visit
                  </Link>
                </div>
              </div>
...
export default FeaturedDrsSlider;
