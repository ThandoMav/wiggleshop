...
export default function AdminFreebieEditScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data } = await axios.get(`/api/admin/freebies/${productId}`);
  ...
  const submitHandler = async ({
    ...
    try {
      ...
      await axios.put(`/api/admin/freebies/${productId}`, {
        name,
        tag,
        slug,
        downloadFile,
  ...
  return (
    <Layout title={`Edit freebie ${productId}`}>
        ...
        <div className="md:col-span-3">
          {loading ? (
            ...
            >
              ...
              <div className="mb-4">
                <label htmlFor="downloadFile">downloadFile</label>
                <input
                  type="text"
                  className="w-full"
                  id="downloadFile"
              ...
              <div className="mb-4">
                <Link href={`/admin/freebies`}>Back</Link>
              </div>
            </form>
          )}
        </div>
...
AdminFreebieEditScreen.auth = { adminOnly: true };
