...
function CartScreen() {
  ...
  const {
    cartService: { cartServiceItems },
  } = state;
  const removeItemHandler = (item) => {
    dispatch({ type: 'CART_SERVICE_REMOVE_ITEM', payload: item });
  };
  const updateCartHandler = async (item, qty) => {
    ...
    dispatch({ type: 'CART_SERVICE_ADD_ITEM', payload: { ...item, quantity } });
            ...
            <ul>
              <li>
                <div className="pb-3 text-xl">
                  Subtotal ({cartServiceItems.reduce((a, c) => a + c.quantity, 0)}) : $
                  {cartServiceItems.reduce((a, c) => a + c.quantity * c.price, 0)}
                </div>
              </li>
              <li>
                <button
                  onClick={() => router.push('login?redirect=/appointmentshipping')}
