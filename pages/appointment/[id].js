...
function OrderScreen() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        const { data } = await axios.get(`/api/appointments/${orderId}`);
    ...
    return actions.order.capture().then(async function (details) {
      try {
        ...
        const { data } = await axios.put(
          `/api/appointments/${order._id}/pay`,
          details
        );
        dispatch({ type: 'PAY_SUCCESS', payload: data });
        toast.success('appointment order is paid successgully');
      } catch (err) {
  ...
  async function deliverOrderHandler() {
    try {
      ...
      const { data } = await axios.put(
        `/api/admin/appointments/${order._id}/deliver`,
        {}
      );
  ...
  return (
    <Layout title={`appointment ${orderId}`}>
      <h1 className="mb-4 mt-16 text-xl mt-8">{`appointment ${orderId}`}</h1>
      {loading ? (
        <div>Loading...</div>
      ) : error ? (
        <div className="alert-error">{error}</div>
      ) : (
        <div className="grid md:grid-cols-4 md:gap-5 px-16 mt-6 px-6  md:mt-32 md:px-32">
          <div className="overflow-x-auto md:col-span-3">
            <div className="card  p-5">
              ...
              <div className="alert-error flex flex-row gap-2">
                ...
                Text Business details on our whatsApp to listed our:
