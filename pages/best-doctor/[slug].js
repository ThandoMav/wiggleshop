...
import PostDetail from '../../components/BusDetail';
...
const { state, dispatch } = useContext(Store);
  ...
  if (!post) {
   ...
   const addToCartHandler = async () => {
      const existItem = state.cartService.cartServiceItems.find((x) => x.slug === post.slug);
      const quantity = existItem ? existItem.quantity + 1 : 1;
      const { data } = await axios.get(`/api/best-doctors/${post._id}`);
  
      if (data.countInStock < quantity) {
        return toast.error('Sorry. Dr is out of service');
      }
  
      dispatch({ type: 'CART_SERVICE_ADD_ITEM', payload: { ...post, quantity } });
      //router.push('/login?redirect=/appointmentshipping');
      router.push('/cartService');
    };
      ...
      <div className="container mx-auto py-12 my-12 mt-8 mb-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="col-span-1 lg:col-span-8">
            <PostDetail post={post} addToCartHandler={addToCartHandler}/>
            <div className="py-2">
