...
function CartScreen() {
  ...
  const removeItemHandler = (item) => {
    dispatch({ type: 'CART_BUS_REMOVE_ITEM', payload: item });
  };
  const updateCartHandler = async (item, qty) => {
    ...
    dispatch({ type: 'CART_BUS_ADD_ITEM', payload: { ...item, quantity } });
            ...
            <ul>
              <li>
                <div className="pb-3 text-xl">
                  Subtotal ({businessItems.reduce((a, c) => a + c.quantity, 0)}) : $
                  {businessItems.reduce((a, c) => a + c.quantity * c.price, 0)}
                </div>
              </li>
              <li>
                <button
                  onClick={() => router.push('login?redirect=/busshipping')}
